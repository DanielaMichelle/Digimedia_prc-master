...
function enviarEmail(correo) {
    ...
    if (sentMessages.length >= 1 && sentMessages.length < 3) {
        alert("Debe esperar a que se completen los mensajes de correo antes de enviar otro formulario.");
        return;
    }
...
function enviarMensajeCorreo(correo, index, sentMessages) {
    ...
    localStorage.setItem("correoData", JSON.stringify({ sentMessages: sentMessages }));
